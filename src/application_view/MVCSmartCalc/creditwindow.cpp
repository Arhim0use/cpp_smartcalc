...
void CreditWindow::IncertInTable(double &overpayment, double &all_payment) {
  ...
  for (int i = 0; i < table_size; i++) {
    ...
    ui->tableWidget->setItem(
        i, 1, new QTableWidgetItem(QString::number(table[i][1], 'f', 2)));
    ui->tableWidget->setItem(
        i, 2, new QTableWidgetItem(QString::number(table[i][2], 'f', 2)));
    ui->tableWidget->setItem(
        i, 3, new QTableWidgetItem(QString::number(table[i][3], 'f', 2)));
    ui->tableWidget->setItem(
        i, 4, new QTableWidgetItem(QString::number(table[i][4], 'f', 2)));
  }
  ...
  if (ui->radioButton_diff->isChecked())
    ui->label_m_payment->setText(
        ui->label_m_payment->text() + " - " +
...
void CreditWindow::on_pushButton_calculate_clicked() {
  ...
  if (ui->radioButton_ann->isChecked()) {
    controller_.AnnuityPaymentCalc(amout, percent, period,
                                   ui->CB_period->currentIndex());
  } else {
    controller_.DifferentiatedPaymentCalc(amout, percent, period,
                                          ui->CB_period->currentIndex());
  }
