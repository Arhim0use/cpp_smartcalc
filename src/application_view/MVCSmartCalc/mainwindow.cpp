...
void MainWindow::SetUpWindow() {
    ui->graph_widget->yAxis->setRange(ui->DSB_max_y->value(), ui->DSB_min_y->value());
    ui->graph_widget->xAxis->setRange(ui->DSB_max_x->value(), ui->DSB_min_x->value());
...
void MainWindow::FuncButtonClicked() {
    ...
    if (activeLineEdit && activeLineEdit != ui->ResLine && activeLineEdit != ui->Var_line) {
        QString calc_str = activeLineEdit->text();
        calc_str += button->text() + "(";
        ui->pushButton_AC->setText("C");
        activeLineEdit->setText(calc_str);
...
void MainWindow::GrafPlotOnEqClicked(QString &status_str) {
    ...
    if (start_pos >= end_pos || ui->DSB_min_y->value() >= ui->DSB_max_y->value()) {
        status_str += "Error the size of the graph output field";
    } else {
        status_str += "Ok";
        auto my_graph = controller_.GrafCalculation(step_accuracy, start_pos, end_pos);
//        auto my_graph = controller_.GrafCalculation(ui->CalcLine->text().toStdString() ,step_accuracy, start_pos, end_pos);
        PlotGraf(my_graph);
    }
...
void MainWindow::on_pushButton_AC_clicked() {
        if (ui->CalcLine->text().isEmpty() &&
                ui->Var_line->text().isEmpty() &&
