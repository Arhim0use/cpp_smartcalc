...
void SmartCalc::PushOperator(int &priority, std::list<std::pair<double, short>> &stack, 
  ...
  if (!stack.empty() && priority <= GetPriority(stack.back().second)) {
    ...
    do {
      if (!stack.empty()) {
        priority = GetPriority(stack.back().second);
    ...
    } while (cp_pr < priority && status_ == kOk);
    stack.push_back(*it);
  }
  else
  {
...
int SmartCalc::GetPriority(const int token_type) const noexcept {
  ...
  if (token_type == kNumber || token_type == kVariable) {
    ...
    // ( )
    priority = 2;
  } else if (token_type > kVariable && token_type < kMul) {
    // + -
    priority = 3;
  } else if (token_type == kPow) {
    // ^
    priority = 5;
  } else if (token_type > kMinus && token_type < kSin) {
    // * / %
    priority = 4;
  } else if (token_type > kMod && token_type < kL_parenthesis) {
    // funk
    priority = 6;
  }
  return priority;
