...
void SmartCalc::IsValidString(const std::string &str) {
  ...
  for (auto it = lower_str.cbegin(); it != lower_str.end() && status_ == kOk;
       ++it) {
    ...
    if (!IsOperator(*it) && !IsParenthesis(*it) && !IsDigit(*it) &&
        !IsLetter(*it) && !IsDot(*it))
      status_ = kError;
  }
...
void SmartCalc::ParseFirstUnari(std::string::const_iterator *it,
                                const char ch) {
  if (**it == '-') {
    IsValidUnari(ch);
    ++(*it);
...
int SmartCalc::GetType(const char ch) const {
  ...
  for (int i = 0; i < size; i++) {
    if (op[i] == ch) return i + Type::kPlus;
  }
  return 0;
}
...
void SmartCalc::ParseOperator(const std::string &str, int at_pos) {
  ...
  if (status_ == Status::kOk) {
    ...
    if (tp == Type::kMinus &&
        (IsOperator(str[at_pos - 1]) || str[at_pos - 1] == '(')) {
      token_line_pair.push_front({-1, Type::kUnary});
    } else if (tp) {
...
bool SmartCalc::IsFunc(const std::string &str) {
  const int size = 9;
  const std::string function[size] = {"sin",  "cos",  "tan", "asin", "acos",
                                      "atan", "sqrt", "log", "ln"};
  bool result = false;
...
void SmartCalc::ParseToken(const std::string &str, const std::string &token,
